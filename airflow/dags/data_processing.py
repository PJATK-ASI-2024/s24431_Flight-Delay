...
def upload_to_google_sheets(sheet_name, data):
    client = authorize_google_sheets()
    try:
        # Otwórz arkusz i pobierz zakładkę
        sheet = client.open(sheet_name).sheet1
        data = pd.read_json(data).values.tolist()
...
def save_data_to_gsheets(**kwargs):
    ...
    upload_to_google_sheets(sheet_name=model_sheet_name, data=train_data)
    upload_to_google_sheets(sheet_name=training_sheet_name, data=test_data)
